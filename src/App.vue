      ...
      <v-app-bar title="MiniBili" class="px-3">
        ...
        <v-menu open-on-hover>
          <template v-slot:activator="{ props }"
            ><v-btn v-bind="props" prepend-icon="mdi-translate">{{ getLocaleName() }}</v-btn></template
          ><v-list>
            <v-list-item v-for="(item, index) in localeList" :key="index">
              <v-list-item-title>
                <v-btn variant="plain" @click="changeLocale(item.locale)">{{
                  item.name
        ...
        <v-tooltip
          location="bottom"
          :text="theme === 'light' ? 'Toggle on night' : 'Toggle on light'"
        >
          <template v-slot:activator="{ props }">
            <v-btn
              v-bind="props"
