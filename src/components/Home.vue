...
<template>
  <v-container
    ><v-infinite-scroll mode="manual" :items="recommendations" :onLoad="load">
      <template v-for="n in Math.floor(recommendations.length / 3)" :key="n">
        <v-row no-gutters>
          <v-col
          ...
          >
            <v-skeleton-loader
              :loading="getRealIndex(n - 1, 3, i) >= recommendations.length"
